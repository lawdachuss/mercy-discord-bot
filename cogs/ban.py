...
class BanCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.collection = self.bot.mongo_client['discord_bot']['ban_config']
        self.config_cache = {}
    ...
    async def load_config_cache(self):
        try:
            ...
            self.config_cache = {str(doc['guild_id']): (doc['command'], doc['response']) for doc in docs}
            self.cache_ready.set()
        except:
            self.cache_ready.set()
    ...
    async def setban(
        ...
        try:
            ...
            await self.collection.update_one(
                ...
                {"$set": {"command": command, "response": placeholder_response}},
                upsert=True
            )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not config:
            try:
                doc = await self.collection.find_one({"guild_id": message.guild.id})
                if doc:
                    config = (doc['command'], doc['response'])
