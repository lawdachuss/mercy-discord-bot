...
class AutoResponder(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot
        self.collection = self.bot.mongo_client['discord_bot']['autoresponses']
    ...
    async def _get_guild_autoresponses(self, guild_id):
        ...
        try:
            cursor = self.collection.find({"guild_id": str(guild_id)}).sort("_id", 1)
    ...
    async def _add_autoresponse(self, guild_id, trigger, response, user_id):
        ...
        try:
            ...
            existing = await self.collection.find_one(
                {"guild_id": str(guild_id), "trigger": {"$regex": f"^{re.escape(trigger)}$", "$options": "i"}}
            )
            if existing:
    ...
    async def add_response(self, ctx, trigger: str, *, response: str):
        ...
        success, message = await self._add_autoresponse(guild_id, trigger, response, ctx.author.id)
